...
export class AIKernel implements IAIKernel {
  ...
  public async processPrompt(
    ...
    // Workspace path should only be required for project-editing intents.
    const isProjectEditing = intent === 'MODIFY_PROJECT' || intent === 'DEBUG_PROJECT' || intent === 'EXPLAIN_CODE';
    if (isProjectEditing && (!effectiveWorkspacePath || effectiveWorkspacePath === '.')) {
      throw new Error('Workspace Detection Error: No workspace folder is open. Please open a workspace to run modify, debug, or explain commands.');
    }
