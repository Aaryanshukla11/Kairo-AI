...
export class OnnxProvider implements ModelProvider {
  public name = 'OnnxProvider';
  ...
  public async validateModel(modelConfig: ModelConfig): Promise<boolean> {
    return !!modelConfig && modelConfig.provider === 'onnx';
  }
  ...
  public async getResourceUsage(): Promise<{ memoryBytes: number; vramBytes: number; cpuPct: number; gpuPct: number }> {
    return {
      memoryBytes: this.isBusy ? 768 * 1024 * 1024 : 96 * 1024 * 1024,
      vramBytes: this.currentModel ? 2048 * 1024 * 1024 : 0,
      cpuPct: this.isBusy ? 40 : 1,
      gpuPct: this.isBusy ? 50 : 0
    };
  }
  ...
  public async generate(
    ...
    const responseText = `[ONNX Runtime response] Generated using ONNX Runtime backend for prompt: ${promptPkg.userPrompt.substring(0, 30)}...`;
